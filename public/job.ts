...
namespace HttpRelay.Proxy {
    ...
    export class Job {
        ...
        public execute(handlerRequest: HandlerRequest, jobId: string): Promise<SerRespInit> {
            let funcRes = this.handler.execute(handlerRequest, this.params)
            if (funcRes instanceof HandlerFuncResult) {
